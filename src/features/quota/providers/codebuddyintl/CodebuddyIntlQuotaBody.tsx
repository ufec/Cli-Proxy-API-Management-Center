/**
 * CodeBuddy Intl 额度渲染体：资源包用量水位条。
 */

import { useTranslation } from 'react-i18next';
import type { CodeBuddyIntlQuotaState } from '@/types';
import { QuotaMeter } from '../../components/QuotaMeter';
import type { QuotaBodyProps } from '../../types';

export function CodebuddyIntlQuotaBody({
  quota,
  classes,
}: QuotaBodyProps<CodeBuddyIntlQuotaState>) {
  const { t } = useTranslation();
  const rows = quota.rows ?? [];

  if (rows.length === 0) {
    return <div className={classes.quotaMessage}>{t('codebuddy_intl_quota.empty_data')}</div>;
  }
